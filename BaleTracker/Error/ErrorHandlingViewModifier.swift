                    ...
                    if case let .retryableConnectionError(_, retryAction) = error {
                        createAlert(content: content,
                                    alert: Alert(
                                        ...
                                        primaryButton: .default(Text(R.string.localizable.btn_retry()), action: {
                                            Task {
                                                try? await retryAction()
                                            }
                                        }),
                                        secondaryButton: .cancel(Text(R.string.localizable.btn_cancel()), action: action)
                                    ))
                    } else {
                        createAlert(content: content,
                                    alert: Alert(
                                        title: Text(title),
                                        message: Text(message),
                                        dismissButton: .default(Text(R.string.localizable.btn_ok()), action: action)
                                    ))
                    }
